...
window.confirmerPaiement = async function() {
  ...
  // Construire la liste des documents + liens
  const lignesDocuments = panier.map(p =>
    `• ${p.icon} ${p.titre} (${p.niveau} · ${p.matiere}) — ${p.prix} FCFA`
  ).join('\n');

  const liensHtml = panier.map(p =>
    `• ${p.icon} <a href="${p.lien_pdf}" target="_blank">${p.titre}</a>`
  ).join('\n');

  const liensTxt = panier.map(p =>
    `• ${p.icon} ${p.titre} : ${p.lien_pdf}`
  ).join('\n');

  // ── 1. Envoyer email automatique si livraison par email ──
  if (livraison === 'email') {
    const btnConfirm = document.querySelector('#page-paiement .btn-primary');
    if (btnConfirm) { btnConfirm.disabled = true; btnConfirm.textContent = '⏳ Envoi en cours...'; }
    ...
    try {
      ...
      showToast('📧 Email envoyé avec tes documents !', '✅');
    } catch (err) {
      console.error('EmailJS error:', err);
      showToast('Erreur email — vérifie ta connexion', '❌');
      if (btnConfirm) { btnConfirm.disabled = false; btnConfirm.textContent = '💬 Confirmer via WhatsApp →'; }
      return;
    }
  }

  // ── 2. Toujours envoyer le message WhatsApp (preuve de paiement) ──
  const whatsappURL = `https://wa.me/${WHATSAPP_NUM}?text=${genererMessageWhatsApp()}`;
  window.open(whatsappURL, '_blank');

  // ── 3. Vider le panier et afficher confirmation ──
  panier = [];
  updateCartCount();
