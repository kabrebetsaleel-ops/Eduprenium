...
window.confirmerPaiement = async function() {
  ...
  try {
    // ── Enregistrer via fetch vers Firestore REST API ──
    const FIREBASE_PROJECT = "exercices-devoirs-annales";
    const commande = {
      fields: {
        nom:         { stringValue: nom },
        tel:         { stringValue: tel },
        moyen:       { stringValue: moyen },
        transaction: { stringValue: transaction },
        livraison:   { stringValue: livraison },
        email:       { stringValue: email || '' },
        note:        { stringValue: note  || '' },
        total:       { integerValue: getTotalPanier() },
        ...
        documents:   {
          arrayValue: {
            values: panier.map(p => ({
              mapValue: {
                fields: {
                  id:      { integerValue: p.id },
                  titre:   { stringValue: p.titre },
                  niveau:  { stringValue: p.niveau },
                  matiere: { stringValue: p.matiere },
                  prix:    { integerValue: p.prix },
                  lien_pdf:{ stringValue: p.lien_pdf || '' },
                }
              }
            }))
          }
    ...
    // Ajouter le timestamp de création
    commande.fields.createdAt = { timestampValue: new Date().toISOString() };

    // Récupérer le token de l'utilisateur connecté (partagé par auth.js via window)
    let fetchHeaders = { 'Content-Type': 'application/json' };
    try {
      const currentUser = window._firebaseAuth ? window._firebaseAuth.currentUser : null;
      if (currentUser) {
        const token = await currentUser.getIdToken();
        fetchHeaders['Authorization'] = 'Bearer ' + token;
      }
    } catch(e) { /* pas de token, on continue */ }

    const res = await fetch(
      `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT}/databases/(default)/documents/commandes`,
      {
        method: 'POST',
